...
- (void)passItemsToCells
{
    [self.itemArray enumerateObjectsUsingBlock:^(CHFChatStackItem *item, NSUInteger idx, BOOL *stop) {
        
        CGPoint point = [self pointForCellAtIndex:idx inSection:0];
        CHFItemCollectionViewCell *cell = [self cellAtIndex:idx inSection:0];
        
        [ChatStackManager snapItem:item
                           toPoint:point
...
{
    NSLog(@"count = %i", self.itemArray.count);
    switch (section)
    {
        case 0:
