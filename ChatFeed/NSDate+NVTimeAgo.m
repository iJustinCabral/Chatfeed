...
- (NSString *)formattedAsCountingTimeAgo
{
    //Now
...
- (NSTimeInterval)secondsSince
{
    return -[self timeIntervalSinceNow];
}

- (NSTimeInterval)minutesSince
{
    return (int)[self secondsSince] / MINUTE;
}
...
#pragma mark TimeAgoTimer Implementation

@interface TimeAgoTimer ()
@property (nonatomic) NSTimer *timer;
@end

@implementation TimeAgoTimer

+ (instancetype)sharedTimeAgoTimer
{
    static id instance;
...
- (instancetype)init
{
    ...
    if (self)
    {
        if (!self.timer)
        {
            self.timer = [NSTimer timerWithTimeInterval:1.0 target:self selector:@selector(secondsNotification) userInfo:nil repeats:YES];
        }
...
- (void)secondsNotification
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"timeAgoSecondNotification"
                                                        object:nil
                                                      userInfo:nil];
}
