...
- (void)tappedItem:(UITapGestureRecognizer *)tapGesture
{
    if (!self.delegate)
    {
        [ChatStackManager addItem:self fromPoint:tapGesture.view.center animated:YES withCompletionBlock:^(BOOL finished) {
...
- (void)pannedItem:(UIPanGestureRecognizer *)panGesture
{
    if (!self.delegate)
    {
        [ChatStackManager passPanningItem:self withPanGesture:panGesture andCompletionBlock:^(BOOL finished) {
